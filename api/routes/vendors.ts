...
router.post("/", requireAuth, async (req: AuthedRequest, res: Response) => {
  ...
  const row = (
    await sql`
      INSERT INTO vendors (org_id, code, name, notes, is_active, created_at)
  ...
  res.status(200).json({ success: true, data: { vendor: row, createdBy: userId } });
});
...
router.patch("/:id", requireAuth, async (req: AuthedRequest, res: Response) => {
  ...
  const sql = getSql();
  const existing = await sql`SELECT id FROM vendors WHERE org_id = ${orgId} AND id = ${id} LIMIT 1`;
  if (!existing.length) {
  ...
  res.status(200).json({ success: true, data: { vendor: next } });
});
